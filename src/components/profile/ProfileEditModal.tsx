...
const ProfileEditModal: React.FC<ProfileEditModalProps> = ({
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const userId = getFirebaseCompatSync().auth().currentUser?.uid;
      ...
      if (field === 'username' && typeof value === 'string') {
        await getFirebaseCompatSync().firestore().runTransaction(async (transaction) => {
          ...
          const newUsernameRef = getFirebaseCompatSync().firestore().collection('usernames').doc(value.toLowerCase());
          ...
          const userRef = getFirebaseCompatSync().firestore().collection('users').doc(userId);
          ...
          if (currentValue && typeof currentValue === 'string' && currentValue !== value.toLowerCase()) {
            const oldUsernameRef = getFirebaseCompatSync().firestore().collection('usernames').doc(currentValue);
        ...
        // Normal güncelleme
        await getFirebaseCompatSync().firestore().collection('users').doc(userId).update(updateData);
  ...
  const filteredListData = useMemo(() => {
    const baseList = getListData();
    if (!listSearchQuery.trim()) {
      return baseList;
    }
    const query = listSearchQuery.toLowerCase();
    return baseList.filter((item: any) => {
      const text = (item.label || item.name || item.value || '').toLowerCase();
      return text.includes(query);
    });
  }, [listSearchQuery, field]);

  const handleItemSelect = (item: any) => {
    const normalizedValue = item?.value ?? item?.id ?? item?.label;
  ...
  const renderListItem = ({ item }: { item: any }) => {
    ...
    >
      <Text style={styles.listItemText}>{displayLabel}</Text>
    </TouchableOpacity>
  );
  };
              ...
              showListSelection ? (
                <View style={styles.listContainer}>
                  ...
                  <FlatList
                    ...
                    keyExtractor={(item, index) => item.id || item.value || item.label || index.toString()}
