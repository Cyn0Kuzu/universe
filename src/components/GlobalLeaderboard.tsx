...
export const GlobalLeaderboard: React.FC<GlobalLeaderboardProps> = ({
  ...
  const loadLeaderboard = useCallback(async (refresh = false) => {
    try {
      ...
      if (userType === 'club') {
        ...
        const clubScoresSnapshot = await firebase.firestore()
        ...
        entries = await Promise.all(
          clubScores.map(async (scoreData: any) => {
            ...
            const userDoc = await firebase.firestore().collection('users').doc(scoreData.clubId).get();
            ...
            if (validatedPoints < 50) {
              ...
              try {
                await firebase.firestore().collection('clubScores').doc(scoreData.clubId).update({
        ...
        entries = await Promise.all(
          centralLeaderboard.map(async (rankData) => {
            // User bilgilerini getir
            const userDoc = await firebase.firestore().collection('users').doc(rankData.userId).get();
  ...
  useEffect(() => {
    ...
    const unsubscribeUsers = firebase.firestore()
      .collection('users')
      .where('userType', '==', userType || 'student')
      .onSnapshot(
        (snapshot) => {
