...
const ClubEventCard = ({ 
  ...
  const coverImageUri = useMemo(() => {
    ...
    const selectedUri = imageFields.find(
      (uri) => uri && typeof uri === 'string' && uri.trim() !== '',
    );
    ...
    return typeof selectedUri === 'string' ? selectedUri : null;
  }, [event]);

  const coverImageSource = coverImageUri
    ? { uri: coverImageUri }
  ...
  React.useEffect(() => {
    const checkUserLiked = async () => {
      ...
      try {
        const likesRef = getFirebaseCompatSync().firestore()
          .collection('events')
          .doc(event.id)
          .collection('likes');
  ...
  React.useEffect(() => {
    ...
    const checkAttendanceStatus = async () => {
      ...
      try {
        ...
        if (currentUser) {
          const attendeeRef = getFirebaseCompatSync().firestore()
            .collection('events')
            .doc(event.id)
          ...
          const enrichedLikesData = await Promise.all(
            likesData.map(async (like: any) => {
              try {
                ...
                const userDoc = await getFirebaseCompatSync().firestore().collection('users').doc(like.userId).get();
          ...
          const enrichedCommentsData = await Promise.all(
            commentsData.map(async (comment: any) => {
              try {
                ...
                const userDoc = await getFirebaseCompatSync().firestore().collection('users').doc(comment.userId).get();
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      const eventRef = getFirebaseCompatSync().firestore().collection('events').doc(event.id);
      ...
      while (retryCount < maxRetries) {
        try {
          await getFirebaseCompatSync().firestore().runTransaction(async (transaction) => {
            ...
            if (existingLikeQuery.size > 0) {
              ...
              try {
                const userDoc = await getFirebaseCompatSync().firestore().collection('users').doc(currentUser.uid).get();
                if (userDoc.exists) {
                  const userData = userDoc.data();
  ...
  const handleAttend = async () => {
    ...
    try {
      const attendeesRef = getFirebaseCompatSync().firestore()
        .collection('events')
        .doc(event.id)
        .collection('attendees');
      ...
      await getFirebaseCompatSync().firestore().runTransaction(async (transaction) => {
        ...
        const attendeesColRef = getFirebaseCompatSync().firestore().collection('events').doc(event.id).collection('attendees');
      ...
      console.error('Error details:', { message: error?.message, code: error?.code });
      Alert.alert('Hata', 'Katılım işlemi sırasında bir hata oluştu. Lütfen tekrar deneyin.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePostComment = async () => {
    ...
    try {
      ...
      */
      ...
      let errorMessage = 'Yorum yapılırken bir sorun oluştu.';
      if (error?.code === 'permission-denied') {
        errorMessage = 'Yorum göndermek için yetkiniz bulunmuyor.';
        ...
        for (const gl of globalLikesSnapshot.docs) {
          ...
          try {
            ...
            await likesCollection.add({
              ...
              avatarColor: userData.avatarColor || '#1976D2',
              timestamp: g.createdAt || getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
            });
          } catch (e) {
      ...
      <Card style={[styles.card, style]} elevation={2}>
        ...
        <TouchableOpacity onPress={toggleExpand} style={{ width: '100%' }}>
          <View style={styles.imageContainer}>
            <Card.Cover source={coverImageSource} style={styles.cardImage} resizeMode="cover" />
            {coverImageUri ? (
              <TouchableOpacity
                style={styles.imagePreviewButton}
                onPress={(eventPress) => {
                  eventPress.stopPropagation();
                  showPreview(coverImageUri, event.title || 'Etkinlik Görseli');
                }}
                activeOpacity={0.85}
              >
