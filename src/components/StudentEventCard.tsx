          ...
          <View style={styles.commentUserDetails}>
            ...
            {!!liveUniversity && (
              <View style={styles.commentUniversityBadge}>
                <MaterialCommunityIcons name="school" size={16} color="#0066cc" />
...
const StudentEventCard: React.FC<StudentEventCardProps> = ({
  ...
  const safeNavigate = useCallback((routeName: string, params?: any) => {
    ...
    try {
      if (navigation && typeof navigation.navigate === 'function') {
        ...
        Alert.alert('Bilgi', 'Navigasyon şu anda kullanılamıyor. Lütfen ana ekrana dönüp tekrar deneyin.');
      }
    } catch (err: any) {
  ...
  const enrichUserProfile = useCallback(async (userId: string, userData: any = {}) => {
    try {
      ...
      const userProfileDoc = await getFirebaseCompatSync().firestore()
        .collection('users')
        .doc(userId)
        .get();
  ...
  useEffect(() => {
    ...
    const attendeesUnsubscribe = getFirebaseCompatSync().firestore()
      .collection('events')
      .doc(event.id)
    ...
    const likesUnsubscribe = getFirebaseCompatSync().firestore()
      .collection('events')
      .doc(event.id)
      .collection('likes')
      .onSnapshot(async (snapshot) => {
    ...
    const commentsUnsubscribe = getFirebaseCompatSync().firestore()
      .collection('events')
      .doc(event.id)
      .collection('comments')
      .onSnapshot(async (snapshot) => {
  ...
  const handleUserProfileNavigation = (userId: string) => {
    ...
    if (navigation) {
      try {
        ...
        console.error('Error details:', { message: error?.message, stack: error?.stack?.substring(0, 200) });
        Alert.alert('Hata', 'Profil sayfasına yönlendirme sırasında bir hata oluştu. Lütfen tekrar deneyin.');
      }
    } else {
        ...
        {(!eventImageUri || imageLoadFailed) ? (
          ...
          <>
            <Image 
              ...
              resizeMode="cover"
              onError={() => {
                console.log('🚨 Image loading failed for event:', event.id, 'URI:', eventImageUri);
              ...
              onLoadStart={() => {
                console.log('🖼️ Image loading started for event:', event.id);
                setImageLoadFailed(false);
              }}
            />
            <TouchableOpacity
