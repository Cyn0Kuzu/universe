...
// Services
import unifiedStatisticsService, { UserStatistics, ClubStatistics } from '../../services/unifiedStatisticsService';
...
const StatisticsLeaderboardScreen: React.FC = () => {
  ...
  // Fetch statistics data using unified statistics service
  const fetchStatisticsData = async () => {
    ...
    try {
      ...
      if (selectedTab === 'users') {
        ...
        const userPromises = usersSnapshot.docs.map(async (doc) => {
          ...
          return {
            ...
            clubs: 0 // Will be calculated if needed
          };
        });
        
        entries = await Promise.all(userPromises);
        console.log(`👥 Found ${entries.length} users with unified statistics`);
        
      } else if (selectedTab === 'events') {
        console.log('🎉 Fetching event statistics using direct queries (no unified service for events yet)...');
        // For events, we still need to use direct queries as the unified service doesn't handle events
        ...
        // Process events in smaller batches to avoid too many concurrent requests
        const batchSize = 10;
        for (let i = 0; i < eventsSnapshot.docs.length; i += batchSize) {
          const batch = eventsSnapshot.docs.slice(i, i + batchSize);
          const batchPromises = batch.map(async (doc) => {
            const eventData = doc.data();
            
            try {
              // Get basic statistics for this event
              ...
              if (eventData.clubId || eventData.organizerId) {
                try {
                  ...
                  if (clubDoc.exists) {
                    ...
                    clubName = clubData?.displayName || clubData?.name || 'Bilinmeyen Kulüp';
                    clubUsername = clubData?.username || '';
                    clubAvatar = clubData?.profileImage || '';
                  }
                } catch (error) {
                  console.warn('Error fetching club data for event:', doc.id, error);
                }
              }
              ...
              return {
                ...
                avatar: eventData.imageUrl || eventData.coverImage || eventData.coverPhoto || null,
        ...
        console.log(`🎉 Processed ${entries.length} events`);
        ...
        const clubPromises = clubsSnapshot.docs.map(async (doc) => {
          ...
          return {
            ...
            likes: statistics.likes,
            comments: statistics.comments,
            participations: statistics.eventsOrganized,
            members: statistics.memberCount,
            followers: clubData.followers?.length || 0
          };
        });
        
        entries = await Promise.all(clubPromises);
        console.log(`🏛️ Found ${entries.length} clubs with unified statistics`);
      }
      ...
      console.log(`✅ StatisticsLeaderboard: Successfully loaded ${entries.length} ${selectedTab} entries`);
      
    } catch (error) {
      console.error('❌ StatisticsLeaderboard: Error fetching statistics:', error);
      ...
      setRefreshing(false);
        ...
        for (const doc of eventsSnapshot.docs) {
          ...
          // Calculate actual statistics from collections
          const [likesSnapshot, commentsSnapshot, attendeesSnapshot] = await Promise.all([
            firebase.firestore().collection('eventLikes').where('eventId', '==', doc.id).get(),
            firebase.firestore().collection('eventComments').where('eventId', '==', doc.id).get(),
            firebase.firestore().collection('eventAttendees').where('eventId', '==', doc.id).get()
          ]);
          
          const actualLikes = likesSnapshot.docs.length;
          const actualComments = commentsSnapshot.docs.length; 
