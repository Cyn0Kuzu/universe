...
const HomeScreen: React.FC = () => {
  ...
  const followedClubListeners = useRef<Record<string, () => void>>({});
  ...
  useEffect(() => {
    return () => {
      Object.values(followedClubListeners.current).forEach((unsubscribe) => {
        if (typeof unsubscribe === 'function') {
          unsubscribe();
        }
      });
      followedClubListeners.current = {};
    };
  }, []);
  
  const attachClubStatsListener = useCallback((clubId: string) => {
    if (followedClubListeners.current[clubId]) {
      return;
    }

    const unsubscribe = firebase.firestore().collection('clubStats').doc(clubId).onSnapshot((doc) => {
      if (doc.exists) {
        ...
        setFollowedClubs((prev) =>
          prev.map((club) =>
            club.id === clubId
              ? {
                  ...club,
                  followerCount: statsData.totalFollowers ?? statsData.followerCount ?? club.followerCount,
                  memberCount: statsData.totalMembers ?? club.memberCount,
                  eventCount: statsData.totalEvents ?? club.eventCount,
                }
              : club
          )
        );
    ...
    followedClubListeners.current[clubId] = unsubscribe;
  }, []);
  ...
  const fetchFollowedClubs = useCallback(async () => {
    ...
    try {
      ...
      Object.keys(followedClubListeners.current).forEach((clubId) => {
        if (!followedClubIds.includes(clubId)) {
          followedClubListeners.current[clubId]?.();
          delete followedClubListeners.current[clubId];
        }
      });
        ...
        try {
          ...
          try {
            ...
            if (statsDoc.exists) {
              ...
              realFollowerCount = statsData?.totalFollowers ?? statsData?.followerCount ?? 0;
          ...
          if (clubDoc.exists) {
            ...
            const fallbackFollowerCount = Array.isArray(club?.followers)
              ? club.followers.length
              : (club?.followerCount || 0);
            const syncedFollowerCount = realFollowerCount > 0 ? realFollowerCount : fallbackFollowerCount;
      ...
      clubsData.forEach((club) => {
        attachClubStatsListener(club.id);
      });
      return clubsData;
    } catch (error) {
  ...
  const handleJoinEvent = async (eventId: string) => {
    ...
    try {
      ...
      await eventRef.update({
        attendees: getFirebaseCompatSync().firestore.FieldValue.arrayUnion(currentUser.uid),
  ...
  const handleUnjoinEvent = async (eventId: string) => {
    ...
    try {
      ...
      await eventRef.update({
        attendees: getFirebaseCompatSync().firestore.FieldValue.arrayRemove(currentUser.uid),
        attendeesCount: getFirebaseCompatSync().firestore.FieldValue.increment(-1)
      });
  ...
  const handleShareEvent = async (eventId: string) => {
    try {
      ...
      if (eventDoc.exists) {
        ...
        await eventRef.update({
          shareCount: getFirebaseCompatSync().firestore.FieldValue.increment(1)
        });
      }
    ...
    useCallback(() => {
      ...
      const loadData = async () => {
        try {
          ...
          // Kritik verileri önce yükle (hızlı gösterim için)
          await fetchFollowedClubEvents();
          
          // Diğer verileri arka planda yükle
          Promise.all([
            fetchJoinedEvents(),
            fetchMemberClubs(),
            fetchUserJoinedEvents(),
            ...
            console.warn('⚠️ Background data fetch error:', error);
          });
          
          console.log('🎉 Critical data loaded, background loading continues');
        } catch (error) {
