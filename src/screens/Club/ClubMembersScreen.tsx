...
const ClubMembersScreen: React.FC = () => {
  ...
  const removeMember = async (memberId: string, userId: string, memberName: string) => {
    Alert.alert(
      'Üyeyi Kaldır',
      `${memberName} adlı üyeyi kulüpten kaldırmak istediğinizden emin misiniz?`,
      [
        { text: 'İptal', style: 'cancel' },
        {
          text: 'Kaldır',
          ...
          onPress: async () => {
            try {
              ...
              // Üyeliği sil
              const memberRef = db.collection('clubMembers').doc(memberId);
              batch.delete(memberRef);
              
              // Kulüp üye sayısını azalt (doğru koleksiyonu kullan)
              // Kulüp verisi users koleksiyonunda ise users'ı, clubs'ta ise clubs'ı kullan
              let clubRef;
              if (clubData?.userType === 'club') {
                // Kulüp users koleksiyonunda
                clubRef = db.collection('users').doc(clubData.id);
              } else {
                // Kulüp clubs koleksiyonunda (eski sistem)
                clubRef = db.collection('clubs').doc(clubData.id);
              }
              
              console.log('Updating club member count for:', clubData.id, 'in collection:', 
                clubData?.userType === 'club' ? 'users' : 'clubs');
              
              // Kulüp dokümanının varlığını kontrol et
              const clubDocExists = await clubRef.get();
              if (clubDocExists.exists) {
                batch.update(clubRef, {
                  memberCount: firebase.firestore.FieldValue.increment(-1),
                });
              } else {
                console.warn('Club document not found for ID:', clubData.id);
                // Eğer kulüp dokümanı bulunamazsa sadece üyeliği sil (memberCount güncelleme atlanır)
              }
              
              await batch.commit();
              
              // Update club stats (decrement member count)
              try {
              ...
              // Add user activity for the club (üyeyi çıkardınız)
              try {
              ...
              // Club member kick statistics are tracked directly in Firebase collections
              console.log('✅ Club member kick statistics recorded');

              // Student club leave statistics are tracked directly in Firebase collections
              console.log('✅ Student club leave statistics recorded');

              // Send notification to the removed member
              try {
                // TODO: Replace with ClubNotificationService
                // await enhancedClubNotificationService.sendMemberKickedNotification(
                //   userId,
                //   clubData?.id || '',
                ...
                console.warn('Member kicked notification failed:', notifErr);
              }

              // Log user activity for the removed member
              try {
            ...
            filteredMembers.map((member) => (
              <MemberCard
                ...
                onRemove={() => removeMember(
                  member.id, 
                  member.userId, 
                  member.userInfo?.displayName || 'İsimsiz Kullanıcı'
                )}
...
const MemberCard: React.FC<MemberCardProps> = ({ member, onRemove, onViewProfile, getUniversityName }) => {
        ...
        >
          ...
          <View style={styles.memberInfo}>
            ...
            {member.userInfo?.department && (
              <Text style={styles.memberDepartment}>{member.userInfo.department}</Text>
            )}
          </View>
        </TouchableOpacity>
      ...
      <View style={styles.requestContainer}>
        ...
        >
          ...
          <View style={styles.requestInfo}>
            ...
            <Text style={styles.requestUniversity}>
