...
import { View, StyleSheet, ScrollView, RefreshControl, ImageBackground, TouchableOpacity, Share, useWindowDimensions, Alert } from 'react-native';
...
const ViewClubScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeFollowerCount = getFirebaseCompatSync().firestore()
      .collection('users')
  ...
  useEffect(() => {
    ...
    const unsubscribeFollowingCount = getFirebaseCompatSync().firestore()
      .collection('users')
      .doc(clubId)
      .onSnapshot(
        (clubDoc) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeMembers = getFirebaseCompatSync().firestore()
  ...
  useEffect(() => {
    ...
    const unsubscribeEvents = getFirebaseCompatSync().firestore()
      .collection('events')
  ...
  const handleMembershipToggle = async () => {
    ...
    try {
      if (isMember) {
        // Kulüpten ayrıl
        const membershipQuery = await getFirebaseCompatSync().firestore()
        ...
        // Kulübe üye ol
        await getFirebaseCompatSync().firestore().collection('clubMemberships').add({
          clubId,
  ...
  const onRefresh = useCallback(async () => {
    ...
    try {
      ...
      await getFirebaseCompatSync().firestore().collection('users').doc(clubId).update({
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <View style={styles.coverSection}>
        ...
        <TouchableOpacity 
          style={[styles.backButton, styles.floatingActionLeft, floatingButtonOffset]}
          onPress={handleGoBack}
        >
        ...
        {/* More Options Button */}
        <TouchableOpacity 
          style={[styles.menuButton, styles.floatingActionRight, floatingButtonOffset]}
          onPress={() => {}}
        >
