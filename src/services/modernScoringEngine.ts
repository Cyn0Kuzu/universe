  ...
  private async createActivity(data: any): Promise<string> {
    try {
      const activity: Partial<ScoreActivity> = {
        ...
        status: 'active',
        timestamp: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp() as any,
    ...
    try {
      ...
      if (userPoints !== 0) {
        ...
        batch.update(userRef, {
          totalScore: getFirebaseCompatSync().firestore.FieldValue.increment(userPoints),
          [`stats.${new Date().toISOString().split('T')[0]}.score`]: getFirebaseCompatSync().firestore.FieldValue.increment(userPoints),
      ...
      if (targetPoints !== 0 && targetType === 'user' && targetId) {
        ...
        batch.update(targetRef, {
          totalScore: getFirebaseCompatSync().firestore.FieldValue.increment(targetPoints),
          [`stats.${new Date().toISOString().split('T')[0]}.score`]: getFirebaseCompatSync().firestore.FieldValue.increment(targetPoints),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
      }
      ...
      if (clubPoints !== 0 && targetType === 'club' && targetId) {
        ...
        batch.update(clubRef, {
          totalScore: getFirebaseCompatSync().firestore.FieldValue.increment(clubPoints),
          [`stats.${new Date().toISOString().split('T')[0]}.score`]: getFirebaseCompatSync().firestore.FieldValue.increment(clubPoints),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
      }
