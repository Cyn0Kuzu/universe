...
class ClubStatsServiceClass {
  async getClubStats(clubId: string): Promise<ClubStatsData | null> {
    try {
      const statsDoc = await getFirebaseCompatSync().firestore()
  ...
  private async ensureStatsDocumentExists(clubId: string): Promise<void> {
    const statsRef = getFirebaseCompatSync().firestore().collection('clubStats').doc(clubId);
  ...
  async incrementEventCount(clubId: string): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalEvents: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          monthlyEvents: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error) {
  ...
  async incrementMemberCount(clubId: string): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalMembers: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          monthlyMembers: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error) {
  ...
  async incrementLikeCount(clubId: string): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalLikes: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          monthlyLikes: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          totalInteractions: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error) {
  ...
  async incrementCommentCount(clubId: string): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalComments: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          totalInteractions: getFirebaseCompatSync().firestore.FieldValue.increment(1),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error) {
  ...
  async updateCommentCount(clubId: string, increment: boolean = true): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalComments: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          totalInteractions: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error) {
  ...
  async updateEventCount(clubId: string, increment: boolean = true): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalEvents: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          monthlyEvents: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          totalInteractions: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error: any) {
  ...
  async resetAllClubStats(): Promise<void> {
    try {
      ...
      // Tüm kulüpleri al
      const usersSnapshot = await getFirebaseCompatSync().firestore()
        .collection('users')
        .where('userType', '==', 'club')
        .get();
  ...
  async updateLikeCount(clubId: string, increment: boolean = true): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalLikes: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error: any) {
  ...
  async updateParticipantCount(clubId: string, increment: boolean = true): Promise<void> {
    try {
      ...
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalParticipants: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          totalInteractions: getFirebaseCompatSync().firestore.FieldValue.increment(incrementValue),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
    } catch (error: any) {
  ...
  async incrementTotalInteractions(clubId: string, amount: number = 1): Promise<void> {
    try {
      await getFirebaseCompatSync().firestore()
        ...
        .doc(clubId)
        .update({
          totalInteractions: getFirebaseCompatSync().firestore.FieldValue.increment(amount),
          lastUpdated: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
        });
  ...
  async syncUserScores(clubId: string): Promise<void> {
    try {
      ...
      const userScoreRef = getFirebaseCompatSync().firestore().collection('userScores').doc(clubId);
