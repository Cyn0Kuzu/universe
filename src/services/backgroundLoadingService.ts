 ...
 * Preloads critical screens and data during the initial loading pass
 */
...
class BackgroundLoadingService {
  ...
  /**
   * Start background preloading during the initial boot window
   */
  ...
  private async preloadUsers(): Promise<void> {
    try {
      ...
      const { currentUser } = getFirebaseCompatSync().auth();
      if (!currentUser) {
  ...
  private async preloadUserProfile(userId: string): Promise<void> {
    try {
      ...
      const doc = await getFirebaseCompatSync().firestore()
        .collection('users')
        .doc(userId)
        .get();
