...
export class ClubScoreFixService {
  ...
  static async ensureClubScoreDocument(clubId: string): Promise<boolean> {
    try {
      ...
      const clubStatsDoc = await getFirebaseCompatSync().firestore().collection('clubStats').doc(clubId).get();
  ...
  const level = Math.floor(calculatedPoints / 1000);
      ...
      const clubScoresRef = getFirebaseCompatSync().firestore().collection('clubScores').doc(clubId);
      ...
      const clubScoreData = {
        ...
        fixedByService: true,
        fixedAt: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp(),
  ...
  static async syncUserScoresWithClubScores(clubId: string): Promise<boolean> {
    try {
      ...
      const clubScoreDoc = await getFirebaseCompatSync().firestore().collection('clubScores').doc(clubId).get();
      ...
      const currentUserScoresDoc = await getFirebaseCompatSync().firestore().collection('userScores').doc(clubId).get();
  ...
  static async fixCompleteClubScore(clubId: string): Promise<{
    ...
    try {
      ...
      const finalClubScoreDoc = await getFirebaseCompatSync().firestore().collection('clubScores').doc(clubId).get();
  ...
  private static async logScoreChange(
    ...
    try {
      ...
      await getFirebaseCompatSync().firestore().collection('scoreChangeLogs').add(logData);
