...
export class ClubFollowSyncService {
  ...
  static async followClub(
    ...
    try {
      ...
      batch.update(userRef, {
        followedClubs: getFirebaseCompatSync().firestore.FieldValue.arrayUnion(clubId),
        followingCount: getFirebaseCompatSync().firestore.FieldValue.increment(1)
      });
      
      batch.update(clubRef, {
        followers: getFirebaseCompatSync().firestore.FieldValue.arrayUnion(userId),
  ...
  static async unfollowClub(
    ...
    try {
      ...
      batch.update(userRef, {
        followedClubs: getFirebaseCompatSync().firestore.FieldValue.arrayRemove(clubId),
        followingCount: getFirebaseCompatSync().firestore.FieldValue.increment(-1)
      });
      
      batch.update(clubRef, {
        followers: getFirebaseCompatSync().firestore.FieldValue.arrayRemove(userId),
        followerCount: getFirebaseCompatSync().firestore.FieldValue.increment(-1)
      });
