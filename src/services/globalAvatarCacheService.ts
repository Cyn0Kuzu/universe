...
class GlobalAvatarCacheService {
  ...
  private setupRealtimeListener(userId: string): void {
    ...
    const unsubscribe = getFirebaseCompatSync().firestore().collection('users').doc(userId)
      .onSnapshot(
        (doc) => {
          if (doc.exists) {
  ...
  async batchCacheUsers(userIds: string[]): Promise<Map<string, UserAvatarData>> {
    ...
    try {
      ...
      const userDocs = await Promise.all(
        userIds.map(userId => getFirebaseCompatSync().firestore().collection('users').doc(userId).get())
      );
