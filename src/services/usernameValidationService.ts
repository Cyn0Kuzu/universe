...
class UsernameValidationService {
  ...
  async checkUsernameAvailability(username: string, currentUserId?: string): Promise<{
    ...
    try {
      ...
      if (currentUserId) {
        try {
          const currentUserDoc = await getFirebaseCompatSync().firestore()
            .collection('users')
            .doc(currentUserId)
            .get();
      ...
      try {
        ...
        try {
          const testDoc = await getFirebaseCompatSync().firestore().collection('usernames').doc('test_doc_access').get();
        ...
        const usernameDocRef = getFirebaseCompatSync().firestore().collection('usernames').doc(normalizedUsername);
  ...
  async validateAndReserveUsername(username: string, userId: string): Promise<{
    ...
    try {
      ...
      const result = await getFirebaseCompatSync().firestore().runTransaction(async (transaction) => {
        const usernameRef = getFirebaseCompatSync().firestore().collection('usernames').doc(normalizedUsername);
