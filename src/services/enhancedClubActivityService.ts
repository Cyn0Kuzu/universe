...
export class EnhancedClubActivityService {
  ...
  private async ensureFirebaseReady(): Promise<void> {
    ...
    if (!this.initializationPromise) {
      this.initializationPromise = (async () => {
        ...
        if (!firebaseCompat) {
          throw new Error('[ActivityService] Firebase SDK not available');
        }
        if (!firebaseCompat.apps?.length) {
          firebaseCompat.initializeApp?.(firebaseConfig);
        }
        if (!firebaseCompat.firestore) {
          throw new Error('[ActivityService] Firebase Firestore compat API not available');
        }
  ...
  private async getDb(): Promise<firebase.firestore.Firestore> {
    ...
    if (!this.db) {
      throw new Error('[ActivityService] Firestore not initialized');
    }
    return this.db;
  }
  ...
  private getFieldValue() {
    if (!this.firebaseCompat?.firestore?.FieldValue) {
      throw new Error('[ActivityService] Firebase FieldValue not available');
    }
  ...
  private getTimestamp() {
    if (!this.firebaseCompat?.firestore?.Timestamp) {
      throw new Error('[ActivityService] Firebase Timestamp not available');
    }
  ...
  async getClubActivities(
    ...
    try {
      ...
      let query = db.collection('clubActivities')
        .where('clubId', '==', clubId);
  ...
  async subscribeToClubActivities(
    clubId: string,
    ...
    try {
      ...
      const limit = filter?.limit || 20;
      const unsubscribe = db.collection('clubActivities')
        .where('clubId', '==', clubId)
        .limit(limit)
        .onSnapshot((snapshot) => {
          const activities: ClubActivity[] = [];
