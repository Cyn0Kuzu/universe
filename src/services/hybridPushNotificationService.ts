...
export class HybridPushNotificationService {
  ...
  async sendToUser(
    ...
    try {
      ...
      const notificationRef = await getFirebaseCompatSync().firestore()
  ...
  async sendToUsers(
    ...
    try {
      ...
      const batchSize = 500;
      const notificationsRef = getFirebaseCompatSync().firestore().collection('notifications');
  ...
  async sendImmediateToUser(
    ...
    try {
      ...
      const sendManualNotification = getFirebaseCompatSync()
        .functions()
      ...
      if (result.data.success) {
        ...
        await getFirebaseCompatSync().firestore().collection('notifications').add({
          userId,
          title: payload.title,
          body: payload.body,
          type: payload.type || 'default',
  ...
  async sendBatchToUsers(
    ...
    try {
      ...
      const sendBatchNotifications = getFirebaseCompatSync()
        .functions()
