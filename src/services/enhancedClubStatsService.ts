...
export interface ClubStats {
  ...
  lastRecalculated?: FirestoreTimestamp;
  isActive: boolean;
  weeklyStats: {
    [week: string]: {
      events: number;
  ...
  async incrementMemberCount(clubId: string): Promise<void> {
    try {
      ...
      await statsRef.update({
        ...
        memberGrowthToday: getFirebaseCompatSync().firestore.FieldValue.increment(1),
        lastUpdated: firebase.firestore.Timestamp.now()
      });
  ...
  async incrementEventCount(clubId: string): Promise<void> {
    try {
      ...
      await statsRef.update({
        ...
        eventsThisMonth: getFirebaseCompatSync().firestore.FieldValue.increment(1),
        lastUpdated: firebase.firestore.Timestamp.now()
      });
