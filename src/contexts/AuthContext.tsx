...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const ensureCorrectUserType = useCallback(async (uid: string) => {
    try {
      ...
      const doc = await getFirebaseCompatSync().firestore().collection('users').doc(uid).get();
      if (!doc.exists) {
        return;
      }
      ...
      if (looksLikeClub && data.accountType === 'club' && data.userType !== 'club') {
        ...
        const firebase = await getFirebase();
        await getFirebaseCompatSync().firestore().collection('users').doc(uid).update({
          userType: 'club',
          accountType: 'club'
        });
  ...
  const signOut = useCallback(async () => {
    try {
      ...
      // Stop all global listeners before clearing sessions to prevent memory leaks
      try {
        globalRealtimeSyncService.stopGlobalSync();
      } catch (error) {
        console.warn('⚠️ Failed stopping global realtime sync during sign out:', error);
      }

      try {
        enhancedRealtimeSyncService.stopGlobalSync();
      } catch (error) {
        console.warn('⚠️ Failed stopping enhanced realtime sync during sign out:', error);
      }

      try {
        universalProfileSyncService.dispose();
      } catch (error) {
        console.warn('⚠️ Failed disposing universal profile sync during sign out:', error);
      }
      ...
      try {
        ...
        console.warn('⚠️ Forced cleanup after failed sign out:', disposeError);
      }
    }
  }, []);
  ...
  // Sync push notification token once the user is authenticated
  useEffect(() => {
    if (!currentUser?.uid) {
      return;
    }
    ...
    const syncPushToken = async () => {
      try {
        ...
        await pushService.syncTokenWithCurrentUser();
      } catch (error) {
        console.warn('⚠️ Failed to sync push token with current user:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      const { listeners } = globalRealtimeSyncService.getSyncStatus();
      if (listeners === 0) {
        globalRealtimeSyncService.startGlobalSync();
      }
    } catch (error) {
      console.warn('⚠️ Failed ensuring global realtime sync is running:', error);
    }
  ...
  useEffect(() => {
    ...
    // Loading'i hemen false yap - başlangıç yükleme ekranı kendi süresini yönetecek
    setLoading(false);
      ...
      try {
        ...
        setTimeout(async () => {
          try {
            ...
            // Delay auth operations until after initial loading completes (2+ seconds)
            setTimeout(async () => {
              try {
  ...
  if (storedSession) {
          ...
          if (persistedUser && (persistedUser as any).isFromPersistentRestore) {
            ...
            if (persistedUser) {
              ...
              try {
                const firebase = await getFirebase();
                const userDoc = await getFirebaseCompatSync().firestore().collection('users').doc(persistedUser.uid).get();
              ...
              setLoading(false);
              authInitialized = true;
            }, 2500); // Delay auth operations until after initial loading completes (2+ seconds)
  ...
  useEffect(() => {
    ...
    const setupRealtimeListener = async () => {
      try {
        const firebase = await getFirebase();
        return getFirebaseCompatSync().firestore().collection('users').doc(uid).onSnapshot(async (snap) => {
          try {
  ...
  const signIn = async (email: string, password: string, rememberMe: boolean = false) => {
    ...
    try {
      ...
      const authInstance = firebaseCompat?.auth?.();

      if (!authInstance || typeof authInstance.signInWithEmailAndPassword !== 'function') {
        throw new Error('Firebase Auth şu anda kullanılamıyor. Lütfen uygulamayı yeniden başlatın.');
      }

      const userCredential = await authInstance.signInWithEmailAndPassword(normalizedEmail, password);
      ...
      await ensureCorrectUserType(user.uid);
        ...
        console.warn('⚠️ ensureCorrectUserType failed during sign-in, continuing with cached data:', repairError);
      }
      ...
      try {
        const typeDoc = await getFirebaseCompatSync().firestore().collection('users').doc(user.uid).get();
        const data: any = typeDoc.data() || {};
        if (data.userType === 'club') {
          repairedType = 'club';
        } else if (data.userType === 'student') {
          repairedType = 'student';
        }
      } catch (typeError) {
        console.warn('⚠️ Unable to refresh userType from Firestore during sign-in:', typeError);
      }
