...
export const markNotificationAsRead = async (
  ...
  try {
    ...
    const result = await SafeNotificationManager.safeUpdateNotification(
      ...
      { 
        read: true, 
        readAt: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
      },
      targetUserType
    );
    
    if (result.success) {
      // AsyncStorage'ı da güncelle - HER ZAMAN kaydet (persistence için kritik)
      try {
        ...
        if (!readIds.includes(notificationId)) {
          ...
          console.log(`💾 AsyncStorage updated: ${readIds.length} total read notifications`);
        } else {
          console.log(`ℹ️ Notification ${notificationId} already in AsyncStorage`);
        }
  ...
  try {
    ...
    for (const [collection, collectionNotifications] of Object.entries(notificationsByCollection)) {
      ...
      const result = await SafeNotificationManager.safeBatchUpdateNotifications(
        ...
        { read: true, readAt: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp() },
        collectionUserType
      );
