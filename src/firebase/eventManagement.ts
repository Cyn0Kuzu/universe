...
export const likeEvent = async (userId: string, eventId: string): Promise<boolean> => {
  try {
    ...
    const likeQuery = await getFirebaseCompatSync().firestore().collection('eventLikes')
      .where('userId', '==', userId)
    ...
    await getFirebaseCompatSync().firestore().collection('eventLikes').add({
      userId,
      eventId,
    ...
    await getFirebaseCompatSync().firestore().collection('events').doc(eventId).update({
      likesCount: getFirebaseCompatSync().firestore.FieldValue.increment(1)
    });
...
export const joinEvent = async (userId: string, eventId: string): Promise<boolean> => {
  try {
    ...
    const attendeeQuery = await getFirebaseCompatSync().firestore().collection('eventAttendees')
      .where('userId', '==', userId)
      .where('eventId', '==', eventId)
      .limit(1)
      .get();
    ...
    await getFirebaseCompatSync().firestore().collection('eventAttendees').add(attendeeData);
    
    // 2. Event subcollection'a da ekle (ClubEventCard için)
    await getFirebaseCompatSync().firestore().collection('events').doc(eventId).collection('attendees').doc(userId).set(attendeeData);

    // Event attendee count'unu artır
    await getFirebaseCompatSync().firestore().collection('events').doc(eventId).update({
      attendeesCount: getFirebaseCompatSync().firestore.FieldValue.increment(1)
    });
...
export const shareEvent = async (userId: string, eventId: string): Promise<boolean> => {
  try {
    ...
    const shareQuery = await getFirebaseCompatSync().firestore().collection('eventShares')
      .where('userId', '==', userId)
      .where('eventId', '==', eventId)
      .limit(1)
      .get();
    ...
    await getFirebaseCompatSync().firestore().collection('eventShares').add({
      userId,
      eventId,
      sharedAt: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
    });

    // Event share count'unu artır
    await getFirebaseCompatSync().firestore().collection('events').doc(eventId).update({
      sharesCount: getFirebaseCompatSync().firestore.FieldValue.increment(1)
    });
...
export const leaveEvent = async (userId: string, eventId: string): Promise<boolean> => {
  try {
    ...
    if (!attendeeQuery.empty) {
      ...
      await getFirebaseCompatSync().firestore().collection('events').doc(eventId).collection('attendees').doc(userId).delete();
...
export const createEventWithScoring = async (
  ...
  try {
    ...
    const docRef = await getFirebaseCompatSync().firestore().collection('events').add(newEventData);
    ...
    try {
      ...
      // Kulüp üyelerini al
      const clubMembersQuery = await getFirebaseCompatSync().firestore()
    ...
    if (eventData.clubId) {
      console.log(`🎪 ÖNEMLİ: Etkinlik oluşturuldu, istatistikleri güncelleme: ${eventData.clubId}`);
        try {
          ...
          const statsDoc = await getFirebaseCompatSync().firestore().collection('clubStats').doc(eventData.clubId).get();
          if (statsDoc.exists) {
          ...
          const finalStats = await getFirebaseCompatSync().firestore().collection('clubStats').doc(eventData.clubId).get();
      ...
      try {
        const clubDoc = await getFirebaseCompatSync().firestore().collection('users').doc(eventData.clubId).get();
        ...
        const membersSnapshot = await getFirebaseCompatSync().firestore()
          .collection('clubMembers')
          .where('clubId', '==', eventData.clubId)
          .get();
...
export const deleteEventSafely = async (eventId: string, userId: string): Promise<boolean> => {
  try {
    ...
    // Sonra etkinliği sil
    await getFirebaseCompatSync().firestore().collection('events').doc(eventId).delete();
    ...
    try {
      if (eventData.clubId) {
        const actorDoc = await getFirebaseCompatSync().firestore().collection('users').doc(userId).get();
...
export const unlikeEvent = async (userId: string, eventId: string): Promise<boolean> => {
  try {
    ...
    if (!likeQuery.empty) {
      ...
      let clubData = null;
      if (eventData.clubId) {
        const clubDoc = await getFirebaseCompatSync().firestore().collection('clubs').doc(eventData.clubId).get();
