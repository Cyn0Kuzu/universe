...
export const sendMembershipRequest = async (
  ...
  try {
    ...
    const membershipRequest: MembershipRequest = {
      ...
      clubId,
      status: 'pending',
      requestDate: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp(),
      message: message || '',
...
export const approveMembershipRequest = async (
  ...
  try {
    ...
    const memberData = {
      ...
      clubId: clubId,
      joinDate: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp(),
      role: 'member',
      status: 'active',
      approvedBy: approverId
    };
...
export const leaveClub = async (
  ...
  try {
    ...
    memberQuery.docs.forEach(doc => {
      batch.update(doc.ref, {
        status: 'left',
        leaveDate: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp()
      });
    });
...
export const kickMember = async (
  ...
  try {
    ...
    memberQuery.docs.forEach(doc => {
      batch.update(doc.ref, {
        status: 'kicked',
        kickDate: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp(),
        kickedBy: kickedBy,
        kickReason: reason || ''
      });
    });
