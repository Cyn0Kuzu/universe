...
export const leaveClub = async (userId: string, clubId: string): Promise<boolean> => {
  try {
    ...
    const memberQuery = await getFirebaseCompatSync().firestore().collection('clubMembers')
      .where('userId', '==', userId)
      .where('clubId', '==', clubId)
      .limit(1)
      .get();
    ...
    if (!memberQuery.empty) {
      ...
      await getFirebaseCompatSync().firestore().collection('clubs').doc(clubId).update({
...
export const unfollowClub = async (userId: string, clubId: string): Promise<boolean> => {
  try {
    ...
    const followQuery = await getFirebaseCompatSync().firestore().collection('clubFollowers')
      .where('userId', '==', userId)
      .where('clubId', '==', clubId)
      .limit(1)
      .get();
...
export const deleteClubSafely = async (clubId: string, userId: string): Promise<boolean> => {
  try {
    ...
    // Kulüp üyelerini sil
    const membersSnapshot = await getFirebaseCompatSync().firestore().collection('clubMembers')
      .where('clubId', '==', clubId)
      .get();
    ...
    const followersSnapshot = await getFirebaseCompatSync().firestore().collection('clubFollowers')
      .where('clubId', '==', clubId)
      .get();
    ...
    const eventsSnapshot = await getFirebaseCompatSync().firestore().collection('events')
      .where('clubId', '==', clubId)
      .get();
    ...
    // Kulübü sil
    batch.delete(getFirebaseCompatSync().firestore().collection('clubs').doc(clubId));
...
export const checkUserClubInteraction = async (userId: string, clubId: string): Promise<{
  ...
  try {
    ...
    // Takip kontrolü
    const followerQuery = await getFirebaseCompatSync().firestore().collection('clubFollowers')
      .where('userId', '==', userId)
      .where('clubId', '==', clubId)
      .limit(1)
      .get();
