...
export const addEventComment = async (
  ...
  try {
    ...
    const currentUser = getFirebaseCompatSync().auth().currentUser;
    ...
    const commentData: CommentData = {
      ...
      createdAt: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp() as any,
      likesCount: 0,
      repliesCount: 0
    };
    ...
    const docRef = await getFirebaseCompatSync().firestore().collection('eventComments').add(commentData);
    ...
    const eventSubcollectionData = {
      ...commentData,
      timestamp: getFirebaseCompatSync().firestore.FieldValue.serverTimestamp(), // timestamp field for ordering
    };
    
    await getFirebaseCompatSync().firestore().collection('events').doc(eventId).collection('comments').doc(docRef.id).set(eventSubcollectionData);
    ...
    await getFirebaseCompatSync().firestore().collection('events').doc(eventId).update({
      commentsCount: getFirebaseCompatSync().firestore.FieldValue.increment(1)
    });
...
export const deleteEventComment = async (
  ...
  try {
    ...
    if (eventId) {
      ...
      commentDoc = await getFirebaseCompatSync().firestore()
        .collection('events')
        .doc(eventId)
        .collection('comments')
        .doc(commentId)
    ...
    if (!commentData) {
      ...
      commentDoc = await getFirebaseCompatSync().firestore().collection('eventComments').doc(commentId).get();
    ...
    if (!commentData && !eventId) {
      ...
      try {
        const eventsSnapshot = await getFirebaseCompatSync().firestore().collection('events').limit(50).get(); // Limit for performance
        
        for (const eventDoc of eventsSnapshot.docs) {
          const subCommentDoc = await getFirebaseCompatSync().firestore()
            .collection('events')
            .doc(eventDoc.id)
            .collection('comments')
            .doc(commentId)
    ...
    try {
      ...
      await getFirebaseCompatSync().firestore().collection('eventComments').doc(commentId).delete();
    ...
    await getFirebaseCompatSync().firestore().collection('events').doc(commentData.eventId).update({
      commentsCount: getFirebaseCompatSync().firestore.FieldValue.increment(-1)
    });

    // Get event and user data for comprehensive processing
    const eventDoc = await getFirebaseCompatSync().firestore().collection('events').doc(commentData.eventId).get();
    const eventData = eventDoc.data();
...
export const likeEventComment = async (
  ...
  try {
    ...
    const likeQuery = await getFirebaseCompatSync().firestore().collection('commentLikes')
    ...
    // Beğeni ekle
    await getFirebaseCompatSync().firestore().collection('commentLikes').add({
      commentId,
      userId,
    ...
    await getFirebaseCompatSync().firestore().collection('eventComments').doc(commentId).update({
    ...
    try {
      ...
      try {
        ...
        const commentDoc = await getFirebaseCompatSync().firestore().collection('eventComments').doc(commentId).get();
...
export const getEventComments = async (
  ...
  try {
    ...
    const querySnapshot = await getFirebaseCompatSync().firestore()
      .collection('events')
      .doc(eventId)
      .collection('comments')
...
export const getEventCommentCount = async (eventId: string): Promise<number> => {
  try {
    ...
    const querySnapshot = await getFirebaseCompatSync().firestore().collection('eventComments')
