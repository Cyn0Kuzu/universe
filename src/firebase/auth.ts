...
export const signIn = async (email: string, password: string, rememberMe: boolean = false): Promise<{success: boolean, error?: string, userCredential?: any}> => {
  try {
    ...
    if (userCredential.user && !userCredential.user.emailVerified) {
      console.warn('⚠️ User signed in but email is not verified:', normalizedEmail);
    ...
    if (error?.code === 'auth/user-not-found') {
      ...
      errorMessage = 'Doğrulama kimliği eksik';
    ...
    } else if (error?.message && error.message.includes('doğrulanmamış')) {
